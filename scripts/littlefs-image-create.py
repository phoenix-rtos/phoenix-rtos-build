...
class PhLFSCreator:
    def __init__(
        ...
        # `read_size` and `prog_size` must be <= `cache_size`. These two settings don't have to
        # be in sync with settings on device.
        self.fs = littlefs.LittleFS(
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        "--cache-size",
        type=lambda x: int(x, 0),
        default=LIBLFS_CACHESIZE_DEFAULT,
        metavar="bytes",
        help="Size of file cache. Must match the cache size setting on device. Workaround for https://github.com/littlefs-project/littlefs/issues/1200 .",
    )
