...
class PloCmdKernel(PloCmdBase):
    ...
    ALIASES: ClassVar[List[str]] = ["kernelimg"]
    device: str
...
class SecurePloScript(PloScript):
    """Full Secure PLO script definition"""
    ...
    def _write_script_signature(self, file: TextIO):
        ...
        r = bytes(reversed(sig_bytes[:self.ecc_curve.bytes]))
        s = bytes(reversed(sig_bytes[self.ecc_curve.bytes:]))
