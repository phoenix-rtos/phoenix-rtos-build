...
# recursively find all default ported libraries
RESOLVED_PORT_LIBS := $(foreach PORT_LIB_DIR, $(call ports_ldir), $(shell find $(PORT_LIB_DIR) -type f -name "*.a"))

# filter out any ported libraries not listed in LIBS
RESOLVED_PORT_LIBS := $(foreach PORT_LIB, \
	$(RESOLVED_PORT_LIBS), \
	$(if $(filter $(basename $(notdir $(PORT_LIB))), $(LIBS)), $(PORT_LIB),))

# remove ported libraries from LIBS
